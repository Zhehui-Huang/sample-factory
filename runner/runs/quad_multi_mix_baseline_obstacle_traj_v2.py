...
_experiment = Experiment(
    'quad_mix_baseline_grav-8_mixed_one_sphere_obstacle',
    QUAD_BASELINE_CLI,
    _params.generate_params(randomize=False),
)

RUN_DESCRIPTION = RunDescription('quads_multi_mix_obst_grav_8a_v115', experiments=[_experiment])
...
# xvfb-run python -m runner.run --run=quad_multi_mix_baseline_obstacle_traj_v2 --runner=processes --max_parallel=4 --pause_between=1 --experiments_per_gpu=1 --num_gpus=4
# Command to use this script on local machine:
# Please change num_workers to the physical cores of your local machine
# python -m runner.run --run=quad_multi_mix_baseline_obstacle_traj_v2 --runner=processes --max_parallel=4 --pause_between=1 --experiments_per_gpu=1 --num_gpus=4
