...
_experiment = Experiment(
    'quad_mix_baseline-hid_size_64-relu-8_mixed_attn',
    QUAD_BASELINE_CLI,
...
# xvfb-run python -m runner.run --run=quad_multi_mix_baseline_deepsets_hid_size_64_relu --runner=processes --max_parallel=4 --pause_between=1 --experiments_per_gpu=1 --num_gpus=4
# Command to use this script on local machine:
# Please change num_workers to the physical cores of your local machine
# python -m runner.run --run=quad_multi_mix_baseline_deepsets_hid_size_64_relu --runner=processes --max_parallel=4 --pause_between=1 --experiments_per_gpu=1 --num_gpus=4
