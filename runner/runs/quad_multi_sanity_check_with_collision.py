...
RUN_DESCRIPTION = RunDescription('quads_multi_sanity_check_with_collision_v112', experiments=[_experiment])
...
# xvfb-run python -m runner.run --run=quad_multi_sanity_check_with_collision --runner=processes --max_parallel=4 --pause_between=1 --experiments_per_gpu=1 --num_gpus=4
# Command to use this script on local machine:
# Please change num_workers to the physical cores of your local machine
# python -m runner.run --run=quad_multi_sanity_check_with_collision --runner=processes --max_parallel=4 --pause_between=1 --experiments_per_gpu=1 --num_gpus=4