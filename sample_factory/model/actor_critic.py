...
class ActorCriticSeparateWeights(ActorCritic):
    ...
    def _core_rnn(self, head_output, rnn_states):
        ...
        if isinstance(head_output, PackedSequence):
            ...
            unpacked_head_output_split = unpacked_head_output.split([self.actor_core.get_in_size(), self.critic_core.get_in_size()], dim=2)
            head_outputs_split = [
            ...
            head_outputs_split = head_output.split([self.actor_core.get_in_size(), self.critic_core.get_in_size()], dim=1)
            rnn_states_split = rnn_states.chunk(num_cores, dim=1)
    ...
    def forward_tail(
        ...
        core_outputs = core_output.split([self.actor_core.get_out_size(), self.critic_core.get_out_size()], dim=1)
